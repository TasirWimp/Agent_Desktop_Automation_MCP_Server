...
For real click/type providers, the provider binding is paired with agent-authored app-scope binding evidence. After inspecting the latest screenshot artifact, the client records `bindingEvidence` through `desktop_submit_interaction_evidence`, confirming that the visible surface is still the expected app-under-test and not a stale, unrelated, or suspiciously small child surface. The server validates freshness, scope, provider identity, frame hashes, and basic geometry; it does not analyze pixels.
...
- require fresh app-scope binding evidence for real provider click/type actions,
...
`desktop_submit_interaction_evidence` is the preferred operational evidence helper for click targeting. It can record the fresh perception digest, app-scope binding evidence, workflow-state claim, follow-up transition assessment, and click-candidate witness after the client has inspected the latest visual artifact. The compact path is `observe -> inspect visual artifact -> submit_interaction_evidence with digest/binding/workflow evidence -> move_mouse -> observe transitionActionId -> submit_interaction_evidence with digest/binding/transition/candidate evidence`. When `transitionAssessment`, workflow postcondition evidence, and `clickCandidate` appear in one helper call for the same movement, semantic landing is processed before workflow postcondition assessment, and `clickCandidate.movementActionId` may be inferred from `transitionAssessment.actionId`. Strict/debug clients can still call `desktop_evaluate_click_candidate` directly after fresh perception/workflow evidence and supported semantic landing assessment, but the strict evaluator does not accept inline transition assessments. Candidate evaluation checks whether the current session has enough scope, frame, cursor, supported semantic landing, no-contradiction, digest, committed workflow-state or bounded workflow revalidation, current binding evidence when real mutation is possible, and risk evidence to request a future app-scoped click. It records audit residue and a hover target witness when ready, and never executes a click. A ready candidate is evidence for targeting quality and workflow readiness, not permission to click outside the licensed app-under-test model.
...
The current scope-binding runtime validates that app-scoped click/type policy has a fresh `boundAppScope`, that the referenced pre-action observation still matches that bound app identity, and that real-mutation providers also have current agent-authored app-scope binding evidence for the same latest screenshot-bearing observation. Real clicking is available only through the explicit app-scoped Windows provider gate. Real typing is available only through the explicit app-scoped Windows generated-test-input provider gate, and credential-like or secret-like text is blocked before provider execution.
