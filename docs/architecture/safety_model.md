...
## CRPM-Compatible Witness-Bound Runtime

The desktop interaction model is compatible with CRPM's governed re-entry framing, but the public MCP protocol keeps repo-local operational names. ADMCP treats the live desktop or app as source state, screenshot artifacts and frame hashes as witnesses, agent-authored digests and workflow claims as witness judgments, and server-recorded interaction evidence as the reviewed carrier that allows the next bounded action.

The agent is responsible for inspecting the current visual artifact and authoring the local scene/workflow interpretation. The server is the path and policy carrier: it enforces scope, freshness windows, transition gates, residue, repair limits, and next re-entry requirements. It does not inspect pixels, run OCR, infer UI meaning, or become the visual meaning authority.

This compatibility protects these design invariants:

- hidden model memory is never canonical desktop state,
- screenshots, cursor positions, hover states, frame hashes, and provider deltas are witnesses, not the whole app state,
- coordinates are action endpoints only, never proof of semantic target correctness,
- every state-changing action must carry explicit witness-bound evidence and leave a transition trace,
- committed workflow state must be distinguished from transient highlights, open dropdowns, and hover-only affordances,
- ambiguity remains visible as residue, repair pressure, or escalation instead of being silently promoted to success.
...
State-changing actions also require fresh agent-authored perception evidence for the latest screenshot-bearing observation before provider execution. The preferred operational path is `desktop_submit_interaction_evidence`, which records the same perception digest and can also record workflow state, transition assessment, and click-candidate evidence in one call. Strict/debug clients may still call `desktop_submit_perception_digest` directly. The server does not inspect pixels, run OCR, or decide what is visible. It enforces temporal and provenance constraints: the digest must reference the latest recorded observation, match the action scope and intended target, bind to the observation frame hashes, and declare that stale carryover was reviewed.
...
After movement, `desktop_submit_interaction_evidence` with `transitionAssessment`, or the strict `desktop_submit_transition_assessment`, must evaluate the follow-up screenshot against the stored relation, candidate, rejected alternative, expected evidence, and contradiction claim. A supported assessment can unlock click-candidate readiness. A contradicted assessment maps to wrong-target repair. An inconclusive assessment maps to repair needed. Click-candidate evaluation, whether reached through the helper or `desktop_evaluate_click_candidate`, treats cursor/candidate proximity as necessary telemetry and requires supported semantic landing evidence with no contradiction before recording a hover target witness. Helper click-candidate evidence must bind to movement evidence through `clickCandidate.movementActionId`, or through same-call inference from `transitionAssessment.actionId`; strict `desktop_evaluate_click_candidate` does not accept inline transition assessments.

## Workflow-State Claims

Click-candidate evaluation, `desktop_click`, and `desktop_type_text` require workflow-state evidence in addition to fresh perception and relational target evidence. The preferred helper can record the workflow claim in the same call as perception evidence; strict/debug clients may call `desktop_submit_workflow_state_claim`. The workflow claim is authored by the client/agent; the server does not inspect pixels, run OCR, or hardcode application workflows. The server normally enforces that the claim references the latest screenshot-bearing observation, the current perception digest, matching scope, matching frame hashes, and an equivalent intended element target.

An older workflow claim may be revalidated only across low-risk non-durable changes: newer observations and audited non-contradicted `move_mouse` probes. Revalidation requires the current latest observation, a current visible non-contradicted perception digest for the same target/scope, a fresh ready workflow claim for the same target/scope, and no intervening click, typing, app launch, file operation, scope exit, risk prompt, wrong-target transition, or repair-needed transition. This allows a target row to be hovered and clicked without requiring a redundant workflow claim after the hover screenshot, but it does not allow workflow state to survive committed UI changes such as dropdown selection clicks or Run execution.
...
`desktop_submit_interaction_evidence` is the preferred mini-agent evidence fast path. It records a perception digest, can record a workflow-state claim, can attach a semantic transition assessment, and can run click-candidate evaluation without moving, clicking, typing, launching apps, observing, polling, or inspecting pixels. When one helper call includes workflow postcondition evidence and semantic landing evidence for the same movement, semantic landing is recorded first so workflow postcondition assessment cannot consume the movement gate before hover-witness generation. It is a session-state helper over the same policy paths, not a desktop mutation tool.

`desktop_evaluate_click_candidate` is the strict/debug session-aware targeting-quality gate. It checks that a future click candidate references an active session, allowed click action, fresh recorded observation, fresh perception digest, workflow-state readiness or bounded workflow revalidation, matching scope, frame evidence, cursor/candidate proximity, supported semantic landing assessment, no contradiction, committed workflow readiness or an explicit precondition-commit repair role, and a low-risk packet. It may reuse an older supported movement/hover witness only when the latest observation, perception digest, workflow evidence, cursor point, candidate point, target, and scope revalidate the same target. It records a `click_candidate_evaluated` audit event and a hover target witness when ready. It does not click, move, type, capture new frames, or make real clicking available. For app-scoped real click work, this evidence reduces wrong-target clicks and wrong-workflow clicks and guides repair; it is not the main governance boundary.
...
`desktop_click` is mock-only by default. When the Windows provider is selected with `ADMCP_ENABLE_REAL_OBSERVATION=true` and `ADMCP_ENABLE_REAL_CLICK=true`, it may click only inside the bound app-under-test scope. It requires an active session, reversible `licensedAppScope`, fresh `boundAppScope`, fresh app-scope binding evidence authored after inspecting the latest screenshot artifact, fresh pre-action observation, fresh perception digest, workflow-state evidence, relational evidence with `hover_witness` point provenance, a stored hover target witness matching the click point and workflow context, in-frame point, app-scoped `click` permission, audit logging, and a post-click observation before another non-observe action.

`desktop_type_text` is mock-only by default. When the Windows provider is selected with `ADMCP_ENABLE_REAL_OBSERVATION=true` and `ADMCP_ENABLE_REAL_TYPING=true`, it may type only generated test input inside the bound app-under-test scope. It requires an active session, reversible `licensedAppScope`, fresh `boundAppScope`, fresh app-scope binding evidence authored after inspecting the latest screenshot artifact, fresh pre-action observation, fresh perception digest, workflow-state evidence with `text_entry` or `not_applicable` role, app-scoped `type_text` permission, non-sensitive/test-input classification, audit logging, and a post-type observation before another non-observe action. It blocks credential-like or secret-like text before provider calls and must not store text content in action packets or audit events.
...
For real-mutation providers, `boundAppScope` is necessary but not sufficient. Before real click/type execution, the client must submit current app-scope binding evidence through `desktop_submit_interaction_evidence.bindingEvidence`, tied to the latest screenshot-bearing observation and the same frame hashes. The server does not inspect pixels; it enforces that the agent explicitly confirms the visible app/window surface, preserves the artifact path in audit output, rejects contradicted or stale claims, and treats suspect tiny/child-surface geometry as unsafe for real mutation.
