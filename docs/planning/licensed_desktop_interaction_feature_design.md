...
- Preserve OSWorld-style challenge pressure locally: challenge phenomena, checkpointed protected outcomes, watched-source semantic freshness, first-class `ask` cycles, behavior/failure labels, and side-effect safety reports.
...
- ADMCP-023A Scenario Contract And Carrier Schemas: validate scenario, challenge phenomena, watched sources, checkpointed protected outcomes, target registry, cold-agent protocol guards, tool-to-cycle-kind matrix, cycle, carrier, behavior labels, safety report, closure, and landfall/re-entry schemas without executing desktop actions.
